...
NUM_NEIGHBORS = {
    ...
    ('user', 'active_in', 'community'): [25, 10, 5],
    ('community', 'rev_active_in', 'user'): [25, 10, 5],
}
...
if __name__ == "__main__":
    ...
    TEMP_SUBGRAPH_DIR = os.path.join(DATA_PATH, "gaston_ave_init_gemma_temp_subgraphs_pretrain")
    ...
    # --- Step 0: Cleanup Old Subgraphs (Forces a rebuild if graph schema changed) ---
    ...
    # --- Step 1: Split and Save Subgraphs (Uses directed edges from the base graph) ---
    if not os.path.exists(train_subgraph_path) or not os.path.exists(val_test_subgraph_path):
        ...
        # Defensive check for the critical edge
