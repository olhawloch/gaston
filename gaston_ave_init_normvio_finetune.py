...
# --- Task Configuration ---
WANDB_PROJECT = "GASTON_contr_init_normvio_finetuning"
...
EPOCHS = 50
BATCH_SIZE = 256
CLIP_GRAD_NORM = 0.25       # NOTE: ave_init used 1
DROPOUT_RATE = 0.4
NUM_WORKERS = 8
WEIGHT_DECAY = 1e-4
...
def create_train_val_masks(data_object, target_node_type, val_ratio, seed, sentinel):
    ...
    """
    num_nodes = data_object[target_node_type].x.size(0) # Use .x.size(0) for safety
    labels = data_object[target_node_type].y

    # Adjust sentinel type to match labels if necessary
    ...
    try:
        # Try to stratify (should work for binary classification)
...
class LitFineTuningModel(pl.LightningModule):
    ...
    def _common_step(self, batch, phase):
        ...
        loss = self.loss_fn(logits, seed_node_labels.float()) # Loss remains the same (weighted BCE)
    ...
    def on_fit_start(self):
        if self.trainer.train_dataloader is not None and self.trainer.max_epochs > 0:
            try:
                # Estimate total steps more reliably (like in dreaddit/hateful script)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune HGT for Normvio classification (Contr_Init).")
    ...
    print("--- Initializing Normvio Binary Classification Fine-Tuning Process (Contr_Init) ---")
