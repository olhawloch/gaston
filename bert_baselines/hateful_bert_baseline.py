...
# Hyperparameters (Matched to HGT Hateful Script)
BATCH_SIZE = 128
LEARNING_RATE = 1e-4 
EPOCHS = 30
HIDDEN_DIM = 256
...
class LitBaselineClassifier(pl.LightningModule):
    def __init__(self, input_dim: int, hidden_dim: int, learning_rate: float, 
                 weight_decay: float, dropout_rate: float):
        super().__init__()
        ...
        # Standard BCE Loss (Matches HGT script which has no pos_weight)
        self.loss_fn = nn.BCEWithLogitsLoss()
        print("INFO: Using standard BCEWithLogitsLoss (no class weighting).")
    ...
    def configure_optimizers(self):
        ...
        # Scheduler to match HGT
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, 
            T_max=1, 
            eta_min=1e-7 # Matches HGT
        )
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train a baseline text classifier from BERT embeddings for Hateful.")
    ...
    wandb_logger = WandbLogger(
        project="GASTON_hateful_binary_finetuning", # Matches HGT Project
