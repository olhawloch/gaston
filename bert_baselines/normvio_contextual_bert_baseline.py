...
class LitContextualBaseline(pl.LightningModule):
    def __init__(self, 
        ...
        # Fusion Architecture: Concat -> Linear -> ReLU -> Dropout -> Linear
        self.fusion_layer = nn.Linear(text_dim + context_dim, hidden_dim)
        self.activation = nn.ReLU()
        self.dropout = nn.Dropout(dropout_rate)
        self.output_layer = nn.Linear(hidden_dim, 1)

        # Weighted BCE Loss (Dynamic weight passed from main)
    ...
    def on_fit_start(self):
        if self.trainer.train_dataloader and self.trainer.max_epochs > 0:
            ...
            if self.lr_schedulers():
                # Handle single scheduler vs list
...
def load_and_prep_contextual_data(
    ...
    """
    Merges BERT embeddings with Subreddit2Vec embeddings via community edge file.
    """
    ...
    tensor_labels = torch.tensor(df_merged['label'].values, dtype=torch.long)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Train Contextual Baseline for NormVio.")
    ...
    parser.add_argument("--task_name", required=True, help="Name of the task for logging.")
    ...
    # --- Stratified Split (Matches NormVio BERT Baseline) ---
    train_indices = np.arange(len(train_dataset))
    # Extract labels (tensors[2] is the label tensor in our TensorDataset)
    train_labels_all = train_dataset.tensors[2].numpy()
    
    print(f"\n--- Creating stratified train/val split ({1-VAL_SPLIT_RATIO:.2f}/{VAL_SPLIT_RATIO:.2f}) ---")
    ...
    print(f"Splits created: {len(train_subset)} train, {len(val_subset)} val")

    # --- Calculate Dynamic Pos Weight (Matching NormVio BERT Baseline) ---
    # Extract labels from the *subset* indices
    subset_labels = train_dataset.tensors[2][train_idx]
    
    num_negatives = (subset_labels == 0).sum().item()
    num_positives = (subset_labels == 1).sum().item()

    if num_positives == 0:
        print("WARNING: No positive samples found in training set. Using 1.0 weight.")
    ...
    model = LitContextualBaseline(
        ...
        pos_weight=dynamic_pos_weight # Use the dynamically calculated weight
    )

    # --- Trainer ---
    wandb_logger = WandbLogger(
        project=f"GASTON_normvio_contextual_finetuning",
