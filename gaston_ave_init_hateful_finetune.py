...
WANDB_PROJECT = "GASTON_contr_init_hateful_finetuning"
...
BATCH_SIZE = 128
CLIP_GRAD_NORM = 0.25
DROPOUT_RATE = 0.2 # Start with lower dropout
NUM_WORKERS = 8
WEIGHT_DECAY = 1e-4
...
# --- Utility Function: Train/Validation Masking (Stratified) ---
def create_train_val_masks(data_object, target_node_type, val_ratio, seed, sentinel):
    """
    Creates train/validation masks directly on the target node type,
    using stratification.
    """
    num_nodes = data_object[target_node_type].x.size(0)
    labels = data_object[target_node_type].y

    # Ensure sentinel is an integer to match long labels
    current_sentinel = int(sentinel)
    ...
    try:
        ...
        print(f"--- Stratification failed ({e}). Creating random train/val split. ---")
        train_idx, val_idx = sk_train_test_split(
            labeled_indices.cpu().numpy(),
            test_size=val_ratio,
            random_state=seed
...
class NodeClassificationHead(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.lin1(x).relu()
        x = self.dropout(x)
        # Squeeze the final output for binary classification
        return self.lin2(x).squeeze(1)

class LitFineTuningModel(pl.LightningModule):
    def __init__(self, in_channels, metadata, learning_rate, pos_weight, pretrained_model_path=None):
        super().__init__()
        self.save_hyperparameters(ignore=["metadata", "in_channels"]) # Also ignore in_channels
        ...
        if pretrained_model_path:
            ...
            print("WARNING: No pre-trained model path provided. Training HGT from scratch.")

        self.classification_head = NodeClassificationHead(
            HIDDEN_CHANNELS, 
            HEAD_HIDDEN_CHANNELS, 
            1, # Output dim is 1 for binary
            DROPOUT_RATE
        )
    ...
    def _common_step(self, batch, phase):
        ...
        if self.target_node_type not in node_embeddings_dict:
            # print(f"Warning: Target node type '{self.target_node_type}' not found...") # Optional
            self.log(f'{phase}_loss', 0.0, on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
            return None

        batch_size = batch[self.target_node_type].batch_size
        if batch_size == 0:
            # print(f"Warning: Batch size is 0...") # Optional
            self.log(f'{phase}_loss', 0.0, on_step=False, on_epoch=True, prog_bar=True, batch_size=1)
            return None

        # 3. Get seed text embeddings and labels
        seed_text_embeddings = node_embeddings_dict[self.target_node_type][:batch_size]
        seed_node_labels = batch[self.target_node_type].y[:batch_size]

        # 4. Pass ONLY text embeddings to the classification head
        logits = self.classification_head(seed_text_embeddings) # <-- Use text embeddings directly

        # 5. Filter unlabeled, calculate loss and metrics (remains the same)
        # Assuming hateful dataset is fully labeled, mask might not filter anything
        labeled_mask = seed_node_labels != UNLABELED_SENTINEL
        if labeled_mask.sum() == 0:
            # This shouldn't happen if data is fully labeled and batch_size > 0
            print(f"Warning: No labeled nodes found in this {phase} batch after filtering.") # Optional
            return None

        logits = logits[labeled_mask]
        seed_node_labels = seed_node_labels[labeled_mask]

        loss = self.loss_fn(logits, seed_node_labels.float()) # Loss remains the same (standard BCE)

        probs = torch.sigmoid(logits)
        metrics = getattr(self, f"{phase}_metrics")
        if probs.numel() > 0:
             try:
                 metrics.update(probs, seed_node_labels.int())
             except Exception as e:
                 print(f"Error updating binary metrics in {phase}: {e}")
                 pass
    ...
    def on_fit_start(self):
        if self.trainer.train_dataloader is not None and self.trainer.max_epochs > 0:
            try:
                # Estimate total steps more reliably (like in dreaddit script)
                ...
                if self.lr_schedulers():
                    ...
                    print("INFO: No LR scheduler found to set T_max for.")
            except Exception as e:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune HGT for Hateful classification (Contr_Init).") # <-- Desc Updated
    ...
    print("--- Initializing Hateful Binary Classification Fine-Tuning Process (Contr_Init) ---")
    ...
    print("\n--- Recalculating Class Imbalance on Final Train Mask ---")
    train_mask = train_data[TARGET_NODE_TYPE].train_mask
    train_labels = train_data[TARGET_NODE_TYPE].y[train_mask]
    ...
    print(f"  Final Labeled Train Nodes: {train_labels.numel()}")
