...
# --- Task Configuration ---
WANDB_PROJECT = "GASTON_ave_init_normvio_binary_finetuning"
...
class NodeClassificationHead(torch.nn.Module):
    ...
    def forward(self, x):
        x = self.lin1(x).relu()
        x = self.dropout(x)
        # Squeeze the final output for binary classification/regression
        return self.lin2(x).squeeze(1)

# --- Lightning Module ---
class LitFineTuningModel(pl.LightningModule):
    def __init__(self, metadata, learning_rate, pos_weight, pretrained_model_path=None):
        super().__init__()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune HGT for Normvio classification.")
    ...
    print("--- Initializing Normvio Binary Classification Fine-Tuning Process ---")
    ...
    early_stop_callback = EarlyStopping(
        monitor='val_BinaryAUROC',  # Monitor the same metric
    ...
    early_stop_callback = EarlyStopping(
        monitor='val_BinaryAUROC',  # Monitor the same metric as checkpoint
        mode='max',                 # We want to maximize AUROC
        patience=5,                 # Stop after 5 epochs of no improvement
        verbose=True                # Print a message when stopping
    )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[checkpoint_callback, early_stop_callback], # <-- ADDED
