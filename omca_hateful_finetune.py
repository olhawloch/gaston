...
def create_train_val_masks(data_object, target_node_type, val_ratio, seed, sentinel):
    ...
    print(f"  Train Labeled Nodes: {train_mask.sum().item()} (Hate: {train_labels.sum()}, NotHate: {train_mask.sum().item() - train_labels.sum()})")
    print(f"  Validation Labeled Nodes: {val_mask.sum().item()} (Hate: {val_labels.sum()}, NotHate: {val_mask.sum().item() - val_labels.sum()})")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Fine-tune HGT for Hateful classification.")
    ...
    print("--- Initializing Hateful Binary Classification Fine-Tuning Process ---")
    ...
    wandb_logger = WandbLogger(
        project="GASTON_hateful_binary_finetuning",
        log_model=False,
